 ...
 * resolved @path with whatever policy is appropriate.
 *
 * Returns a held sock, or an ERR_PTR.
 */
static struct sock *unix_lookup_bsd_path(const struct path *path, int type)
{
 ...
 */
static int kernel_unix_connect_direct(struct sock *other, struct socket *sock,
				      int flags)
{
	struct sock *sk = sock->sk;
	struct unix_connect_state st = {};
